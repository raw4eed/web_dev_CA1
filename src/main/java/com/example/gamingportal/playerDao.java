...
public class playerDao {
    ...
    public static boolean existsByTag(String tag) throws SQLException {
        try (Connection cn = db.getConnection();
             PreparedStatement ps = cn.prepareStatement(
                     "SELECT 1 FROM players WHERE gamer_tag=?")) {
            ps.setString(1, tag);
    ...
    public static boolean register(String tag, String rawPassword) throws SQLException {
        ...
        try (Connection cn = db.getConnection();
             PreparedStatement ps = cn.prepareStatement(
                     "INSERT INTO players (gamer_tag, password_hash, credits) VALUES (?, ?, 500)")) {
            ps.setString(1, tag);
            ps.setString(2, hash);
    ...
    public static player findByTag(String tag) throws SQLException {
        try (Connection cn = db.getConnection();
             PreparedStatement ps = cn.prepareStatement(
                     "SELECT * FROM players WHERE gamer_tag=?")) {
            ps.setString(1, tag);
    ...
    public static int getCredits(String tag) throws SQLException {
        try (Connection cn = db.getConnection();
             PreparedStatement ps = cn.prepareStatement(
                     "SELECT credits FROM players WHERE gamer_tag=?")) {
            ps.setString(1, tag);
    ...
    public static void addCredits(String tag, int amount) throws SQLException {
        try (Connection cn = db.getConnection();
             PreparedStatement ps = cn.prepareStatement(
                     "UPDATE players SET credits=credits+? WHERE gamer_tag=?")) {
            ps.setInt(1, amount);
            ps.setString(2, tag);
            ps.executeUpdate();
        }
